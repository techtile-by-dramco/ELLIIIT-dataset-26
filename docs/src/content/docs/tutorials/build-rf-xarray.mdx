...
The runtime logs have already been parsed into NetCDF/xarray files such as those below. Analysis still requires validity filtering and the physical conventions described in [Assumptions and Limitations](../../reference/assumptions-and-limitations/):
...
Use the [complete-array and AP-subset examples](../../reference/interpretation-and-joins/#filter-valid-rf-snapshots) to apply the CSI and position masks together and align receiver geometry. Selecting cycles with `.any(dim="hostname")` only establishes that some receivers are present; it does not produce complete 42-AP snapshots. Never replace missing CSI with zeros.

The reconstructed complex quantity retains the acquisition phase convention and receiver IQ amplitude scale. Cable-phase correction is already applied; RF phase-center correction is not. See [Assumptions and Limitations](../../reference/assumptions-and-limitations/) before treating CSI as a geometric channel model.
