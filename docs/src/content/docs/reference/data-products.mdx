...
Availability flags identify populated records; they do not encode per-entry SNR, phase uncertainty, or LoS quality. Some physical conventions are absent from the current variable attributes. Read [Assumptions and Limitations](../assumptions-and-limitations/) for the CSI phase sign, amplitude scale, 920 MHz carrier, and RF antenna reference point before using a geometric model.
