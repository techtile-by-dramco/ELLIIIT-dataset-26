---
title: RF Post-Processing
description: Active extractor path, resulting dataset schema, and the published notebook workflow.
status: stable
component: rf
---

import { publicUrl } from "../../../utils/public-url";

This page is mainly for maintainers and operators.

If you already received a processed `.nc` file and only want to analyze the data, start in the tutorial track instead:

- [Use the RF Xarray](../../tutorials/build-rf-xarray/)
- [Notebook: RF Xarray Structure](../../tutorials/notebook-xarray-structure/)

The active RF post-processing path is:

- `processing/parsing/extract_csi_from_smb_v2.py`

It performs:

1. scan each hostname folder for RF result files
2. parse JSON, JSON-lines, text, YAML, CSV, or legacy archive formats
3. recover `experiment_id`, `cycle_id`, `hostname`, `pilot_phase`, and `pilot_amplitude`
4. apply the cable correction from `client/ref-RF-cable.yml`
5. join with rover positions from `server/record/data/exp-*-positions.csv`
6. drop consecutive duplicate-position cycles using `grid.min_spacing / 5` per axis
7. write one NetCDF dataset

The resulting CSI already includes cable-phase correction. It retains the reference-minus-received phase convention and receiver IQ amplitude scale. The parser does not apply the approximate RF phase-center height offset, remove multipath, or turn partial snapshots into complete arrays. Users must still apply the [validity filters](../interpretation-and-joins/#filter-valid-rf-snapshots) and follow [Assumptions and Limitations](../assumptions-and-limitations/) for model-specific processing.

Useful commands:

```bash
python processing/parsing/extract_csi_from_smb_v2.py
python processing/parsing/extract_csi_from_smb_v2.py --max-measurements 10
python processing/parsing/extract_csi_from_smb_v2.py --output results/csi_custom.nc
python docs/scripts/export_notebook_movies.py
```

The resulting dataset contains:

- coordinates: `experiment_id`, `cycle_id`, `hostname`
- rover variables: `rover_x`, `rover_y`, `rover_z`, `position_available`
- RF variables: `csi_real`, `csi_imag`, `csi_available`
- dataset attributes with coverage counts, duplicate-position filter metadata, and last-measurement metadata

The recommended analysis workflow is now published as notebook-backed docs pages:

- [Notebook: RF Xarray Structure](../../tutorials/notebook-xarray-structure/)
- [Notebook: Acoustic Xarray Structure](../../tutorials/notebook-acoustic-xarray-structure/)
- [Notebook: RF Dataset Overview](../../tutorials/notebook-overview/)
- [Notebook: Rover Positions](../../tutorials/notebook-rover-positions/)
- [Notebook: CSI Per Position](../../tutorials/notebook-csi-per-position/)
- [Notebook: RF And Acoustic At One Position](../../tutorials/notebook-rf-acoustic-position/)
- [Notebook: CSI Movies](../../tutorials/notebook-csi-movies/)

Example videos generated from the merged-dataset movie tutorial:

<video
  controls
  src={publicUrl("media/phase_rover_merged.mp4")}
  style="width: 100%; border-radius: 1rem; margin-bottom: 1rem;"
></video>

<video
  controls
  src={publicUrl("media/power_rover_merged.mp4")}
  style="width: 100%; border-radius: 1rem;"
></video>
