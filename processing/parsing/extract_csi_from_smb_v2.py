...
EXPERIMENTS = ["EXP003", "EXP005", "EXP006", "EXP007", "EXP008", "EXP009", "EXP010", "EXP011", "EXP012"]  # Set to [] or None to include all experiments by default.
...
def build_dataset(
    ...
    csi_pair_count = int(np.count_nonzero(np.any(csi_available > 0, axis=2)))
    csi_measurement_count = int(np.count_nonzero(csi_available > 0))
    logger.info(
        "Building xarray dataset: experiments=%d cycles=%d hostnames=%d csi_pairs=%d host_measurements=%d",
        len(experiment_ids),
        len(cycle_ids),
        len(hostnames),
        csi_pair_count,
        csi_measurement_count,
    )
    dataset_attrs = {
        ...
        "csi_pair_count": csi_pair_count,
        "csi_measurement_count": csi_measurement_count,
        **position_coverage,
    }
...
def write_measurement_count_plot(dataset: xr.Dataset, output_path: Path) -> Path | None:
    try:
        import matplotlib
        ...
        logger.warning("Skipping measurement-count plot because matplotlib is unavailable: %s", exc)
        return None

    csi_available = dataset["csi_available"].values > 0
    measurement_counts = csi_available.sum(axis=2).astype(int)
    if measurement_counts.size == 0 or not np.any(measurement_counts > 0):
        logger.warning("Skipping measurement-count plot because the dataset contains no CSI measurements.")
        return None

    experiment_ids = dataset.coords["experiment_id"].values.astype(str)
    cycle_ids = dataset.coords["cycle_id"].values.astype(int)
    total_measurements_by_experiment = measurement_counts.sum(axis=1).astype(int)
    cycles_with_csi_by_experiment = np.count_nonzero(measurement_counts > 0, axis=1).astype(int)
    total_measurements = int(total_measurements_by_experiment.sum())

    fig_width = max(10.0, min(18.0, 8.0 + 0.015 * max(1, cycle_ids.size)))
    fig, (ax_totals, ax_cycles) = plt.subplots(
        2,
        1,
        figsize=(fig_width, 8.0),
        gridspec_kw={"height_ratios": [1.0, 2.2]},
        constrained_layout=True,
    )

    bar_positions = np.arange(experiment_ids.size, dtype=float)
    bars = ax_totals.bar(
        bar_positions,
        total_measurements_by_experiment,
        color="tab:blue",
        alpha=0.9,
    )
    ax_totals.set_xticks(bar_positions)
    ax_totals.set_xticklabels(experiment_ids)
    ax_totals.set_ylabel("Host measurements")
    ax_totals.set_title(f"CSI measurements in constructed dataset ({total_measurements} total host measurements)")
    ax_totals.yaxis.set_major_locator(MaxNLocator(integer=True))
    ax_totals.grid(True, axis="y", alpha=0.3)
    for bar, total_count, cycle_count in zip(
        bars,
        total_measurements_by_experiment,
        cycles_with_csi_by_experiment,
        strict=True,
    ):
        ax_totals.text(
    ...
    plotted_experiments = 0
    for exp_idx, experiment_id in enumerate(experiment_ids):
        experiment_counts = measurement_counts[exp_idx]
        has_csi_mask = experiment_counts > 0
        if not np.any(has_csi_mask):
            continue
        plotted_experiments += 1
        ax_cycles.plot(
            cycle_ids[has_csi_mask],
            experiment_counts[has_csi_mask],
            marker="o",
            markersize=3,
            linewidth=1.2,
            label=f"{experiment_id} ({int(experiment_counts[has_csi_mask].sum())} measurements)",
        )

    if plotted_experiments == 0:
        plt.close(fig)
        logger.warning("Skipping measurement-count plot because no experiment contains CSI measurements.")
        return None

    ax_cycles.set_xlabel("Cycle ID")
    ax_cycles.set_ylabel("Host measurements per cycle")
    ax_cycles.yaxis.set_major_locator(MaxNLocator(integer=True))
    ax_cycles.grid(True, alpha=0.3)
    if plotted_experiments > 1:
        ax_cycles.legend(loc="upper right", fontsize=9)

    plot_path = output_path.with_name(f"{output_path.stem}_measurement_counts.png")
    plot_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(plot_path, dpi=160)
    plt.close(fig)
    logger.info("Wrote measurement-count plot to %s", plot_path)
    return plot_path
...
def print_extraction_summary(
    ...
    *,
    output_path: Path,
    measurement_plot_path: Path | None,
    data_root: Path,
    positions_root: Path,
    skipped_files: int,
) -> None:
    ...
    if measurement_plot_path is not None:
        print(f"  Measurement count plot: {measurement_plot_path}")
...
def main() -> int:
    ...
    measurement_plot_path = write_measurement_count_plot(dataset, output_path)
    print_extraction_summary(
        csi_rows,
        dataset,
        output_path=output_path,
        measurement_plot_path=measurement_plot_path,
        data_root=data_root,
